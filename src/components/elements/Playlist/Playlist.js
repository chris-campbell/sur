...
class Playlist extends React.Component {
  ...
  trackInfo = (event) => {
    // Added both track and track index to array
    var info = []; 
    // Pushes track data attribute to info array
    info.push(event.target.getAttribute("data-track"));
    // Pushes track index data attribute to info array
  ...
  render() {
    ...
    const listing = tracks.slice(0, this.props.limit).map((track, index) => {
      return <li onClick={this.trackInfo} key={index}><a data-num={index} data-track={track.preview}>{track.title}</a></li>
    })

    return (
      <div>
      <h3>Tracks</h3>
        <ul className="sur-track-listing">
          {listing}
        </ul>
